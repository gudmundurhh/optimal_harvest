...
"""
Computing the optimal harvesting policy for the stochastic Rosenzweig-MacArthur model using the HJB equation with adjusted noise.
"""
...
import time 
from hjb_base import *

start_time = time.time()
V_base, fx_base, fy_base = base() #calling the base model for comparison of value function

name='Adjusted Noise Level'
maxX=0.66 #same optimal harvesting point as in the base model
maxY=0.01
...
# Generating the mesh, calling a fucntion from the script boundary_value.py
nx = ny = 32
lx = ly = 3
...
beta = 3
PrHer = 1e0
sigmax = 1/sqrt(2) #noise adjusted
sigmax0 = 0e-6
...
h = Expression( 'PrHer*sqrt(fx*x[0]) + PrCod * sqrt(fy*x[1])',degree=2,domain=mesh, PrHer=PrHer, fx=fx, PrCod=PrCod, fy=fy) # should I add discounting here? Should be in the a part
...
for j in range(real):
    ...
    for i in range(nsteps):
        #computing the population dynamics for the noisy system
...
#Plotting the value function and the optimal policy for the adjusted noise
myplot(Vs,name)
myplot(project(fx,V),'Prey')
myplot(project(fy,V),'Predator')

#plotting the difference between the adjusted noise case and the base case
myplot(Vs-V_base,r'$V_{noise}-V_{base}$')
myplot(project(fx,V)-project(fx_base,V),r'$fx_{noise}-fx_{base}$')
myplot(project(fy,V)-project(fy_base,V),r'$fy_{noise}-fy_{base}$')
